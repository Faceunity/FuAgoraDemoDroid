...
## 数据文件
目录 faceunity/src/main/assets/ 下的 \*.mp3 为程序的数据文件。数据文件中都是二进制数据，与扩展名无关，用mp3只是为了避免打包时额外的压缩。实际在app中使用时，打包在程序内或者从网络接口下载这些数据都是可行的，只要在相应的函数接口传入正确的二进制数据即可。
...
将 nama.jar 放在工程的 faceunity/libs/ 文件夹下。将对应平台的 libnama.so 拷贝至 faceunity/src/main/jniLibs/ 对应文件夹下。
之后在代码中加入
```Java
